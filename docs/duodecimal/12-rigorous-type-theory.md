...
&\text{有损投影}: \mathrm{toDuodec} \ \text{把 DC 压成} \ C_{12} \ \text{残骸} \ (\text{丢弃来源/相位/时钟/归零}) \\
&\text{截断证据}: \text{残骸呈} \ C_{12} \ \text{形状恰证 DC 被剥本质, 非 DC 即} \ C_{12} \\
...
**DC 不是「抽象群同构于 C₁₂」**。所谓"同构"实为 `toDuodec` 的**有损投影**——它是**截断操作**，不是信息中性的等价。`toDuodec : DuodecPoint → Duodec` 把 `Trit × AlphaPower` 压成 `Fin 12`，**丢弃**了：

- 特征 3 的代数来源（谁来自 GF(3) 加法，不可辨）
- ⟨α⟩ 的乘法结构（旋转群周期 4 的来源丢了，只留"有 4 个值"）
- Frobenius 刚性（σ 相位取逆在 +12 里无对应）
- **相位状态** (2026-09-08)：C₁₂ 的 12 个点不携带 C₄ 位置
- **时钟过程** (2026-09-08)：C₁₂ 只剩"群阶 12"，无 `mixedOp^12` 走圈的逐步演化
- **归零机制** (2026-09-08)：只剩 g¹²=e 一条，丢四种归零的来源

**关键**：残骸呈现 C₁₂ 的形状，恰是**截断的证据**——它证明 DC 被剥掉本质后与裸循环群无法区分，而非 DC 本质上就是 C₁₂。投影方向 = 信息丢失方向。
