...
| 文档 | 说明 |
|------|------|
| [Kakeya-元诊断-连续统病态vs离散自愈.md](Kakeya-元诊断-连续统病态vs离散自愈.md) | ⭐ 挂谷猜想 127页vs2页 — 连续统病态终极实证 |
| [Kakeya-形式化验证报告.md](Kakeya-形式化验证报告.md) | ⭐ 挂谷猜想形式化验证 — 6大核心定理 + M_F编码 + 三重完备性 |
| [重构数学体系进展报告.md](重构数学体系进展报告.md) | ⭐ 全库329模块/73276行 — 22/26大类覆盖, 进度85% |
...
`docs/techniques/` —— **可复用的 Agda 证明技术**（每篇含：适用判据 / 实战证据 / 陷阱 / 配套交付纪律；文中引用的符号名均须能在源码 grep 到）。

| 文档 | 一句话 | 实战战果 |
|------|--------|----------|
| [techniques/pair-popping.md](techniques/pair-popping.md) | **配对弹出**：交换结合和式里的同项（相消型：兼相反数；置换型：仅异位）用「重排 + 相消/换位」归位，不做分情形穷举 | M9 Jacobi **729 → 1 行**；行列式乘法性 **6561 → 1 行**（文件 7216 → 849）；GF(27) 乘法交换律 **729 → 59 行**（文件 1499 → 827） |

配套纪律（「穷举 → 结构证明」替换必须附五项证据）：子句数归零 / 新旧语句逐字 `diff` / 主模块 `exit 0` 且 0 postulate / **具体点独立路径复核**（原穷举本身就是一层验证）/ 下游引用者编译验证。
