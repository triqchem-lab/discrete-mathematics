...
z
ˉ
  是反全纯的，连续平移流（continuous flow）缺乏代数刚性，而形式类比缺乏代数相交数的算术约束。完备框架必须在特征 0 空间中显式构造出具备代数刚性、能够保持整性结构的原生自同构驱动词。（erratum 2026-08：z̄ 是 ℂ 的域自同构，所缺的是幂映射式的算术强制与自同构群唯一性，不是自同构资格；见 docs/cross-level/frobenius-vs-conjugation-erratum.md）
