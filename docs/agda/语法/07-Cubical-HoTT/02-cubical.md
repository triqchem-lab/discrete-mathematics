> **Cubical 类型论** · `cubical`
>
> Cubical 模式：区间/路径类型 PathP、transport、partial、homogeneous composition、Glue 类型、高阶归纳类型（HIT）、索引归纳类型。
>
> 🔗 原文：<https://agda.readthedocs.io/en/latest/language/cubical.html>
>
> **本项目相关性**：★ 本项目数学骨架。HoTT/ 模块全部依赖 Cubical：路径空间编码 T⁶ 拓扑、HIT 表达纤维丛、陈类用 transport，PR #8611 正是修复 Cubical 构造子内射性。

---
# Cubical[](#cubical "Link to this heading")

The Cubical mode extends Agda with a variety of features from Cubical Type Theory. In particular, it adds computational univalence and higher inductive types, hence giving computational meaning to [Homotopy Type Theory and Univalent Foundations](https://homotopytypetheory.org/). The version of Cubical Type Theory that Agda implements is a variation of the [CCHM](#cchm) Cubical Type Theory where the Kan composition operations are decomposed into homogeneous composition and generalized transport. This is what makes the general schema for higher inductive types work, following the [CHM](#chm) paper. There is also a research paper specifically about Cubical Agda at <https://www.doi.org/10.1017/S0956796821000034>.

To use the cubical mode Agda needs to be run with the [\--cubical](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical) command-line-option or with `{-# OPTIONS --cubical #-}` at the top of the file.

There are also two other [variants](#variants) of the cubical mode:

* [\--cubical=erased](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical), which is described [below](#erased-cubical), and
* [\--cubical=no-glue](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical), which allows Cubical features without the [Glue types](#glue-types), described [below](#cubical-without-glue).
...
* `agda/cubical`: originally intended as a standard library for Cubical Agda available at <https://github.com/agda/cubical>. This documentation uses the naming conventions of this library, for a detailed list of all of the built-in Cubical Agda files and primitives see [Appendix: Cubical Agda primitives](#primitives-ref).
* `1lab`: A formalised and cross linked reference resource for cubical methods in Homotopy Type Theory which can be found at <https://1lab.dev/>. Much better documented than the `agda/cubical`library and hence more accessible to newcomers. The sources can be found at <https://github.com/plt-amy/1lab>.
...
Follow the instructions at <https://github.com/agda/cubical> to install the library. In order to make this library visible to Agda add `/path/to/cubical/cubical.agda-lib` to `.agda/libraries` and `cubical` to `.agda/defaults` (where `path/to` is the absolute path to where the `agda/cubical` library has been installed). For details of Agda’s library management see [Library Management](https://agda.readthedocs.io/en/latest/tools/package-system.html#package-system).

Expert users who do not want to rely on `agda/cubical` can just add the relevant import statements at the top of their file (for details see [Appendix: Cubical Agda primitives](#primitives-ref)). However, for beginners it is recommended that one uses at least the core part of the `agda/cubical` library.

## The interval and path types[](#the-interval-and-path-types "Link to this heading")

The key idea of Cubical Type Theory is to add an interval type `I : IUniv` (the reason this is in a special sort `IUniv` is because it doesn’t support the `transp` and `hcomp` operations). A variable `i : I` intuitively corresponds to a point in the [real unit interval](https://en.wikipedia.org/wiki/Unit%5Finterval). In an empty context, there are only two values of type `I`: the two endpoints of the interval, `i0` and `i1`.

i0 : I
i1 : I

Elements of the interval form a [De Morgan algebra](https://en.wikipedia.org/wiki/De%5FMorgan%5Falgebra), with minimum (`∧`), maximum (`∨`) and negation (`~`).
...
## Transport[](#transport "Link to this heading")
...
## Partial elements[](#partial-elements "Link to this heading")
...
The term `partialBool i` should be thought of a boolean with different values when `(i = i0)` and `(i = i1)`. Terms of type `Partial φ A` can also be introduced using a [Pattern lambda](lambda-abstraction.md#pattern-lambda).
...
## Homogeneous composition[](#homogeneous-composition "Link to this heading")
...
## Glue types[](#glue-types "Link to this heading")
...
For more results about Glue types and univalence see the files of Glue types and univalence in the `agda/cubical` library or the `1lab`.

## Higher inductive types[](#higher-inductive-types "Link to this heading")

Cubical Agda also lets us directly define higher inductive types as datatypes with path constructors. For example the circle and [torus](https://en.wikipedia.org/wiki/Torus) can be defined as:
...
A type is a _proposition_ if all of its elements are connected by a path:
...
### Erased constructors[](#erased-constructors "Link to this heading")

In combination with [\--erasure](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-erasure) it can make sense to mark constructors as erased, in particular higher constructors such as `squash`:
...
For many more examples of higher inductive types see the `agda/cubical` library or the `1lab`.

## Indexed inductive types[](#indexed-inductive-types "Link to this heading")
...
In more concrete situations, such as when the indices are constructors of some other inductive type, pattern matching definitions will not compute when applied to transports. For specific unsupported cases, see [What works, and what doesn’t](#cubical-ix-matching).
...
### What works, and what doesn’t[](#what-works-and-what-doesn-t "Link to this heading")
...
Definitions which rely on principles incompatible with Cubical Agda (K, injectivity of type constructors) will never compute on transports. Note that enabling both Cubical and K is not compatible with [\--safe](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-safe).
...
### Modalities & indexed matching[](#modalities-indexed-matching "Link to this heading")

When using indexed matching in Cubical Agda, clauses’ arguments (and their right-hand-sides) need to be transported to account for indexing, meaning that the _types_ of those arguments must be well-formed _terms_.
...
| Current \\ Imported                                                                                            | [\--cubical=no-glue](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical)  | [\--cubical=erased](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical)   | [\--cubical\[=full\]](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical)               |
| -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| [\--cubical=no-glue](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical)  | ![\checkmark](https://agda.readthedocs.io/en/latest/_images/math/c4cd8b4fa9fecc5d7dffe984106cc6ec0c1dd9e2.svg) | ![\times](https://agda.readthedocs.io/en/latest/_images/math/f6b3f57626feef2092c2116f3ec0ce719d8aa905.svg)     | ![\times](https://agda.readthedocs.io/en/latest/_images/math/f6b3f57626feef2092c2116f3ec0ce719d8aa905.svg)                   |
| [\--cubical=erased](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical)   | ![\checkmark](https://agda.readthedocs.io/en/latest/_images/math/c4cd8b4fa9fecc5d7dffe984106cc6ec0c1dd9e2.svg) | ![\checkmark](https://agda.readthedocs.io/en/latest/_images/math/c4cd8b4fa9fecc5d7dffe984106cc6ec0c1dd9e2.svg) | ![\checkmark](https://agda.readthedocs.io/en/latest/_images/math/c4cd8b4fa9fecc5d7dffe984106cc6ec0c1dd9e2.svg) [\[1\]](#id7) |
| [\--cubical\[=full\]](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical) | ![\checkmark](https://agda.readthedocs.io/en/latest/_images/math/c4cd8b4fa9fecc5d7dffe984106cc6ec0c1dd9e2.svg) | ![\checkmark](https://agda.readthedocs.io/en/latest/_images/math/c4cd8b4fa9fecc5d7dffe984106cc6ec0c1dd9e2.svg) | ![\checkmark](https://agda.readthedocs.io/en/latest/_images/math/c4cd8b4fa9fecc5d7dffe984106cc6ec0c1dd9e2.svg)               |

\[[1](#id6)\] 

only if [\--erasure](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-erasure) is enabled and is used in erased positions. See [below](#erased-cubical).

### Cubical Agda with erased Glue[](#cubical-agda-with-erased-glue "Link to this heading")

The option [\--cubical=erased](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical) enables a variant of Cubical Agda in which Glue (and the other builtins defined in `Agda.Builtin.Cubical.Glue`) must only be used in [erased](runtime-irrelevance.md#runtime-irrelevance) settings.

Regular Cubical Agda code can import code that uses [\--cubical=erased](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical). Regular Cubical Agda code can also be imported from code that uses [\--cubical=erased](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical), but names defined using Cubical Agda can only be used if the option [\--erasure](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-erasure) is used. In that case the names are treated as if they had been marked as erased, with an exception related to pattern matching:

* Matching on a non-erased imported constructor does not, on its own, make Agda treat the right-hand side as erased.

The reason for this exception is that it should be possible to import the code from modules that use [\--cubical](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical), in which the non-erased constructors are not treated as erased.
...
### Cubical Agda without Glue[](#cubical-agda-without-glue "Link to this heading")

The option [\--cubical=no-glue](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical) enables a variant (strict subset) of Cubical Agda, in which primitives such as `hcomp` and `transp` are still available, but Glue types (and the other builtins defined in `Agda.Builtin.Cubical.Glue`) are disabled. Therefore, it should be sound to postulate in this variant either the uniqueness of identity proofs (UIP) or univalence; but of course not both. A source of inspiration for a Cubical Type Theory compatible with UIP is [XTT](#xtt), in which UIP holds definitionally.

If the current module enables the option [\--cubical=no-glue](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical), then:

* It cannot import from modules with the options [\--cubical](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical) or [\--cubical=erased](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical), since they allow the use of Glue types (to different extents).
* Modules that depend on the current module must enable any of the Cubical (variant) options: [\--cubical=no-glue](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical), [\--cubical=erased](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical), or [\--cubical](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical).

On the other hand, if the current module enables any of the the opions [\--cubical=erased](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical) or [\--cubical](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical), one can always import modules with [\--cubical=no-glue](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-cubical).

## References[](#references "Link to this heading")

> Cyril Cohen, Thierry Coquand, Simon Huber and Anders Mörtberg; [“Cubical Type Theory: a constructive interpretation of the univalence axiom”](https://arxiv.org/abs/1611.02108).

> Thierry Coquand, Simon Huber, Anders Mörtberg; [“On Higher Inductive Types in Cubical Type Theory”](https://arxiv.org/abs/1802.01170).

> Jonathan Sterling, Carlo Angiuli, Daniel Gratzer; [“A Cubical Language for Bishop Sets”](https://dblp.org/rec/journals/lmcs/SterlingAG22.html).

## Appendix: Cubical Agda primitives[](#appendix-cubical-agda-primitives "Link to this heading")
