> **覆盖检查** · `coverage-checking`
>
> 确保函数对所有可能输入模式完备覆盖，含 copattern 与索引数据类型的覆盖性检查规则。
>
> 🔗 原文：<https://agda.readthedocs.io/en/latest/language/coverage-checking.html>
>
> **本项目相关性**：GF(3) Trit 三态枚举与十二律损益链的完备 case 必须通过覆盖检查；缺失分支会阻塞编译。

---
# Coverage Checking[](#coverage-checking "Link to this heading")
...
## Single match on a non-indexed datatype[](#single-match-on-a-non-indexed-datatype "Link to this heading")

When a [function definition](function-definitions.md#function-definitions) pattern matches on a single argument of a simple (i.e. non-indexed) [datatype](data-types.md#data-types), there should be a clause for each constructor. For example:
...
Alternatively, one or more cases may be replaced by a _catchall clause_ that uses a variable pattern or a wildcard pattern \_. In this case, the catchall clause should be last.
...
When the –exact-split flag is enabled, catchall clauses should be marked explicitly by a [catchall pragma](function-definitions.md#catchall-pragma)(`{-# CATCHALL #-}`).
...
In the special case of a datatype with no constructors (i.e. an empty type), there should be a single _absurd clause_ with an [absurd pattern](function-definitions.md#absurd-patterns) () and no right-hand side.
...
## Matching on multiple arguments[](#matching-on-multiple-arguments "Link to this heading")
...
## Copattern matching[](#copattern-matching "Link to this heading")

Functions that return an element of a [record type](record-types.md#record-types) can use [copatterns](copatterns.md#copatterns) to give the individual fields. The coverage check will ensure that there is a single case for each field of the record type. For example:
...
## Matching on indexed datatypes[](#matching-on-indexed-datatypes "Link to this heading")
...
In the special case where unification ends in a conflict for _all_constructors, there should be a single absurd clase (as for an empty type). For example:
...
In the general case, the coverage checker constructs a [case tree](function-definitions.md#case-trees) from the definition given by the user. It then ensures that the following properties are satisfied:
...
For a full formal description of the algorithm that Agda uses to construct a case tree and check coverage of definitions by pattern matching, refer to the article [Elaborating dependent (co)pattern matching: No pattern left behind](https://www.cambridge.org/core/journals/journal-of-functional-programming/article/elaborating-dependent-copattern-matching-no-pattern-left-behind/F13CECDAB2B6200135D45452CA44A8B3).