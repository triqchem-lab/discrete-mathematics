...
>
> 原始递归、结构递归、with-function 的终止性、相关 pragma 与选项。
>
> 🔗 原文：<https://agda.readthedocs.io/en/latest/language/termination-checking.html>
>
> **本项目相关性**：LCM 模运算、损益链递归必须通过终止检查；遇 OOM 时可参考一阶谓词化隔离。

---
# Termination Checking[](#termination-checking "Link to this heading")
...
## Primitive recursion[](#primitive-recursion "Link to this heading")
...
## Structural recursion[](#structural-recursion "Link to this heading")
...
## With-functions[](#with-functions "Link to this heading")

## Pragmas and Options[](#pragmas-and-options "Link to this heading")

* The `NON_TERMINATING` pragma  
This is a safer version of [TERMINATING](#terminating-pragma)which doesn’t treat the affected functions as terminating. This means that `NON_TERMINATING` functions do not reduce during type checking. Though, they do reduce at run-time and when invoking `C-u C-c C-n` interactively. The pragma was added in Agda 2.4.2.
...
The pragma must precede a function definition or a mutual block. The pragma cannot be used in [\--safe](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-safe) mode.  
Examples:
  ...
  * Skipping an old-style mutual block: Before mutual keyword:  
  ...
  mutual  
    ...
    d : A  
    d = c
  * Skipping an old-style mutual block: Somewhere within mutualblock before a type signature or first function clause:  
  mutual  
   {-# TERMINATING #-}  
   e : A  
   e = f  
  ...
  * Skipping a new-style mutual block: Anywhere before a type signature or first function clause in the block:  
  ...
  {-# TERMINATING #-}  
  h = g
* Increasing the maximal analysis depth with [\--termination-depth](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-termination-depth).  
...
mutual  
  ...
  g : Nat → Nat  
  g y = f (suc y)  
With a termination depth of 1, the termination checker would only register that the call from `f`to `g` _decreases_ the argument and the call from `g` to `f` _increases_ the argument, but not by _how much_. Thus, it has no evidence that the call sequence `f → g → f` decreases the argument.  
...
In general termination depth _N_ can track decrease up to _N_ and increase up to _N-1_.  
Agda will first check termination with a termination depth of 1 and increase this value until the termination check succeeds or the maximum termination depth has been reached. The maximum depth is by default 3 (since Agda 2.9.0) and can be set by the [\--termination-depth](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-termination-depth).  
...
[Andreas Abel, Foetus – termination checker for simple functional programs](https://andreasabel.github.io/foetus-report)