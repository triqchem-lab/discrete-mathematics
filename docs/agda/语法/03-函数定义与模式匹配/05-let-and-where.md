> **局部定义 let 与 where** · `let-and-where`
>
> let 表达式、where 块的局部绑定与作用域，以及用 where 证明性质的技巧。
>
> 🔗 原文：<https://agda.readthedocs.io/en/latest/language/let-and-where.html>
>
> **本项目相关性**：构造性证明中常用 where 块组织辅助引理；本项目大量 ≡⟨⟩ 等式链依赖 where 辅助。

---
# Local Definitions: let and where[](#local-definitions-let-and-where "Link to this heading")
...
## let-expressions[](#let-expressions "Link to this heading")
...
However, it is possible to match on _record_ types in the left-hand side of a let-bound function, as described [below](#let-record-pattern):
...
The [internal syntax used by Agda](core-language.md#core-language) does not have let-expressions as a construct. As a result, Agda inlines all let-bound variables during type checking. In particular, let-binding cannot be used to introduce sharing.

### Let binding record patterns[](#let-binding-record-patterns "Link to this heading")
...
This is not allowed if `R` is declared `coinductive`. If `R` lacks eta-equality, e.g. by being declared with `no-eta-equality`, then warning [ShouldBeEtaRecordPattern](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-arg-ShouldBeEtaRecordPattern) is triggered (since Agda 2.9.0).

### Let binding record patterns[](#let-open "Link to this heading")

Let-expressions can be used to locally open a [module](module-system.md#module-system). For example:

let z = x + y
    open M z
in  u          -- using definitions from M

## where-blocks[](#where-blocks "Link to this heading")
...
### Scope of the local declarations[](#scope-of-the-local-declarations "Link to this heading")
...
## Proving properties[](#proving-properties "Link to this heading")
...
## More Examples (for Beginners)[](#more-examples-for-beginners "Link to this heading")
