...
>
> interleaved mutual 块、前向声明、旧式 mutual 块。
>
> 🔗 原文：<https://agda.readthedocs.io/en/latest/language/mutual-recursion.html>
>
> **本项目相关性**：十二律损益链、T⁶ 与 A4 的相互引用、状态机与不变量的互定义场景常用。

---
# Mutual Recursion[](#mutual-recursion "Link to this heading")

Agda offers multiple ways to write mutually-defined data types, record types and functions.

* [Old-style mutual blocks](#mutual-recursion-mutual-block)
* [Forward declaration](#mutual-recursion-forward-declaration)
* [Interleaved mutual blocks](#mutual-recursion-interleaved-mutual)
...
## Interleaved mutual blocks[](#interleaved-mutual-blocks "Link to this heading")
...
The `interleaved mutual` blocks get desugared into the [Forward declaration](#mutual-recursion-forward-declaration) blocks described below by:
...
## Forward declaration[](#forward-declaration "Link to this heading")
...
Such a separation of declaration and definition is for instance needed when defining a set of codes for types and their interpretation as actual types (a so-called _universe_):
...
In contrast to [Interleaved mutual blocks](#mutual-recursion-interleaved-mutual), in forward-declaration style we can only have one `data ... where`block per data type.
...
## Old-style `mutual` blocks[](#old-style-mutual-blocks "Link to this heading")
...
Using the `mutual` keyword, the _universe_ example from above is expressed as follows:
...
This alternative syntax desugars into the new syntax by sorting the content of the mutual block into a _declaration_ and a _definition_part and placing the declarations before the definitions.

_Declarations_ comprise:

* Type signatures of functions, `data` and `record` declarations, `unquoteDecl`. (_Function_ includes here `postulate` and `primitive` etc.)
