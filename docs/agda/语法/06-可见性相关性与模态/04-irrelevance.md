> **无关性（证明无关）** · `irrelevance`
>
> .（dot/irrelevant）标注使参数对计算透明但类型检查保留，用于证明无关、记录字段无关等。
>
> 🔗 原文：<https://agda.readthedocs.io/en/latest/language/irrelevance.html>
>
> **本项目相关性**：命题证明（如 PhaseFamily 不变性）可作为无关参数传递，避免证明项参与计算/引发归约爆炸。

---
# Irrelevance[](#irrelevance "Link to this heading")

Since version 2.2.8 Agda supports irrelevancy annotations. The general rule is that anything prepended by a dot (`.`) is marked irrelevant, which means that it will only be typechecked but never evaluated or compared for equality. Arguments marked as irrelevant are erased by the [compiler](https://agda.readthedocs.io/en/latest/tools/compilers.html#ghc-backend).

Irrelevance annotations are enabled by default. You can use the options –irrelevance/–no-irrelevance to enable/disable them.

Note

This section is about compile-time irrelevance. Agda also supports a weaker form of irrelevance called [Run-time Irrelevance](runtime-irrelevance.md#runtime-irrelevance) that can be used for arguments that should be erased by the compiler but can still be relevant at compile-time.

Note

The [Prop](prop.md#prop) universe provides an alternative form of irrelevance that does not require marking function arguments and declarations as irrelevant.

## Motivating example[](#motivating-example "Link to this heading")
...
## Irrelevant function types[](#irrelevant-function-types "Link to this heading")
...
### What can be done to irrelevant arguments[](#what-can-be-done-to-irrelevant-arguments "Link to this heading")
...
### What can’t be done to irrelevant arguments[](#what-can-t-be-done-to-irrelevant-arguments "Link to this heading")
...
**Example 4.** We also can’t match on an irrelevant record (see [Record Types](record-types.md#record-types)).
...
## Irrelevant declarations[](#irrelevant-declarations "Link to this heading")
...
## Irrelevant record fields[](#irrelevant-record-fields "Link to this heading")

Record fields (see [Record Types](record-types.md#record-types)) can be marked as irrelevant by prefixing their name with a dot in the definition of the record type. Projections for irrelevant fields are only created if option [\--irrelevant-projections](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-irrelevant-projections) is supplied (since Agda > 2.5.4).
...
## Dependent irrelevant function types[](#dependent-irrelevant-function-types "Link to this heading")
...
## Irrelevant instance arguments[](#irrelevant-instance-arguments "Link to this heading")

Contrary to normal instance arguments, irrelevant instance arguments (see [Instance Arguments](instance-arguments.md#instance-arguments)) are not required to have a unique solution.
