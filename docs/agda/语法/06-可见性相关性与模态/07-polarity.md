...
> @++/@+/@-/@ω 等极性模态，与正定性检查的关系。
>
> 🔗 原文：<https://agda.readthedocs.io/en/latest/language/polarity.html>
>
> **本项目相关性**：为高阶参数标注极性可改善类型检查性能并编码参数的角色（协变/逆变）。

---
# Polarity Annotations[](#polarity-annotations "Link to this heading")

Agda supports explicitly annotating functions arguments and datatype parameters with their polarities, using a [modality system](modalities.md#modalities), which the positivity checker can then use to infer positivity. This experimental feature has to be enabled by the [\--polarity](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-polarity) option. Here are some sample uses of the different annotations which type-check:
...
The standard use-case is defining the fix-point of any strictly positive type-former (which is already the criterion for an inductive type to be definable in Agda, see [Strict positivity](data-types.md#strict-positivity), but the polarity modality internalizes this criterion into the type system):
...
## The polarity modality[](#the-polarity-modality "Link to this heading")
...
Strictly positive types are a syntactical condition described for example in section 2.3 of [\[1\]](#polarity-refs). A very similar system (without strict positivity) was described in [\[2\]](#polarity-refs), but didn’t use the modality formalism.
...
\[2\] Andreas Abel, “Polarized Subtyping for Sized Types”, In: Mathematical Structures in Computer Science, 2006, <https://doi.org/10.1007/11753728%5F39>
