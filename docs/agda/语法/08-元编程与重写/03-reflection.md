> **反射（元编程）** · `reflection`
>
> 内建 Reflection 类型、TC 单子、quote/unquote、宏，用于编译期生成/检查代码。
>
> 🔗 原文：<https://agda.readthedocs.io/en/latest/language/reflection.html>
>
> **本项目相关性**：可编写宏自动生成 CRT 投影、批量验证不变量；高级用法，本项目 HoTT 模块可借助。

---
# Reflection[](#reflection "Link to this heading")

## Builtin types[](#builtin-types "Link to this heading")
...
To define a decidable propositional equality with the option [\--safe](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-safe), one can use the conversion to a pair of built-in 64-bit machine words
...
which can be used to define a decidable propositional equality with the option [\--safe](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-safe).
...
### Name abstraction[](#name-abstraction "Link to this heading")
...
There is a built-in type `AGDADEFINITION` representing definitions. Values of this type is returned by the `AGDATCMGETDEFINITION` built-in [described below](#reflection-tc-monad).
...
### Type errors[](#type-errors "Link to this heading")

Type checking computations (see [below](#reflection-tc-monad)) can fail with an error, which is a list of `ErrorPart`s. This allows metaprograms to generate nice errors without having to implement pretty printing for reflected terms.
...
### Blockers[](#blockers "Link to this heading")
...
### Type checking computations[](#type-checking-computations "Link to this heading")
...
## Metaprogramming[](#metaprogramming "Link to this heading")

There are three ways to run a metaprogram (`TC` computation). To run a metaprogram in a term position you use a [macro](#macros). To run metaprograms to create top-level definitions you can use the `unquoteDecl` and `unquoteDef` primitives (see [Unquoting Declarations](#id3)).

### Macros[](#macros "Link to this heading")
...
> * Macros and `quoteTerm` can get blocked on quoting a term with an interactive hole inside it. Enable [\--quote-metas](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-quote-metas) to disable this blocking, so that quoted holes can be constrained by macro unquoting.
...
### Unquoting Declarations[](#id3 "Link to this heading")
...
### System Calls[](#system-calls "Link to this heading")
...
It would be ill-advised to allow Agda to make arbitrary system calls. Hence, the feature must be activated by passing the [\--allow-exec](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-allow-exec) option, either on the command-line or using a pragma. (Note that [\--allow-exec](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-allow-exec) is incompatible with [\--safe](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-safe).) Furthermore, Agda can only call executables which are listed in the list of trusted executables, `~/.agda/executables`. For instance, to run the example above, you must add `/bin/echo` to this file:
...
### Quote Metas[](#quote-metas "Link to this heading")

The option [\--quote-metas](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-quote-metas) enables term quotation constraints to be resolved even when the term has meta variables in it. This allows for typed hole-driven development for macros, where the expected type for an interactive hole given as a quoted argument to a macro can be constrained by the unquoting of the macro.

In the following example without, [\--quote-metas](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-quote-metas) , the interactive hole will cause the constraint to be blocked, which looks something like `quoteTerm ?0 : Term (blocked on _17)`.
...
However, actually unquoting the macro would reveal that the interactive hole should have type `ℕ`. Enabling [\--quote-metas](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-quote-metas) allows exactly this.
