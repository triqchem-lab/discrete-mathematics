> **重写规则** · `rewriting`
>
> 全局 --rewriting + REWRITE pragma 声明用户定义重写规则，含 confluence 检查与导入。
>
> 🔗 原文：<https://agda.readthedocs.io/en/latest/language/rewriting.html>
>
> **本项目相关性**：为 GF(3) 代数恒等式或 CRT 分解添加重写规则可简化证明，但破坏 canonicity 需谨慎。

---
# Rewriting[](#rewriting "Link to this heading")

Rewrite rules allow you to extend Agda’s evaluation relation with new computation rules.

Rules are safe to use with `Agda.Builtin.Equality`if [–confluence-check](#confluence-check) is enabled. Confluent but non-terminating rewrite rules can not break consistency, unlike to non-terminating functions. Those results were proven by [Cockx, Tabareau, and Winterhalter](https://hal.science/hal-02901011v2/document), see section 3 for statements.

Note

This page is about the [\--rewriting](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-rewriting) option and the associated [REWRITE](built-ins.md#builtin-rewrite) builtin. You might be looking for the documentation on the [rewrite construct](with-abstraction.md#with-rewrite) instead.

## Rewrite rules by example[](#rewrite-rules-by-example "Link to this heading")

To enable rewrite rules, you should run Agda with the flag [\--rewriting](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-rewriting)and import the modules `Agda.Builtin.Equality` and `Agda.Builtin.Equality.Rewrite`:
...
### Overlapping pattern matching[](#overlapping-pattern-matching "Link to this heading")
...
Additional examples of how to use rewrite rules can be found in [a blog post by Jesper Cockx](https://jesper.sikanda.be/posts/hack-your-type-theory.html).

### Controlling rewrite rule matching with `primRewriteNoMatch`[](#controlling-rewrite-rule-matching-with-primrewritenomatch "Link to this heading")
...
### Definitional singletons and subject reduction[](#definitional-singletons-and-subject-reduction "Link to this heading")
...
Unfortunately, this causes subject reduction issues when eliminating into `⊤`. Note that, by η-equality, we have `circle-elim (λ _ → ⊤) tt p = circle-elim (λ _ → ⊤) tt q`definitionally for any two identity proofs `p` and `q`.
...
The computation rule `circle-elim-loop` should therefore also imply `p ≡ q` definitionally, but the inductive identity type does not satisfy such an η rule.
...
## General shape of rewrite rules[](#general-shape-of-rewrite-rules "Link to this heading")
...
Once a rewrite rule has been added, Agda automatically rewrites all instances of the left-hand side to the corresponding instance of the right-hand side during reduction. More precisely, a term (definitionally equal to) `f p₁σ ... pₙσ` is rewritten to `vσ`, where `σ` is any substitution on the pattern variables `x₁`, … `xₖ`.

Since rewriting happens after normal reduction, rewrite rules are only applied to terms that would otherwise be neutral.

## Confluence checking[](#confluence-checking "Link to this heading")

Agda can optionally check confluence of rewrite rules by enabling the [\--confluence-check](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-confluence-check) flag. Concretely, it does so by enforcing two properties:
...
> 2. Each rewrite rule should satisfy the _triangle property_: For any rewrite rule `u = w` and any single-step parallel unfolding `u => v`, we should have another single-step parallel unfolding `v => w`.

There is also a flag [\--local-confluence-check](https://agda.readthedocs.io/en/latest/tools/command-line-options.html#cmdoption-confluence-check) that is less restrictive but only checks local confluence of rewrite rules. In case the rewrite rules are terminating (currently not checked), these two properties are equivalent.
...
## Importing rewrite rules[](#importing-rewrite-rules "Link to this heading")
...
In summary, the scoping rules for rewrite rules are same as the scoping rules for instances in [Haskell 2010](https://www.haskell.org/onlinereport/haskell2010/haskellch5.html#x11-1060005.4).