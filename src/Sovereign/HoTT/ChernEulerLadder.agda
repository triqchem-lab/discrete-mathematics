...
--   有限格点上的真实 K 理论已形式化于 Sovereign.Algebra.DiscreteKTheory:
--   K₀(pt)≅ℤ (Grothendieck 完备化) + 陈特征环 ℤ[ε]/(ε²) + Bott 周期 (C₂×C₃=C₆)
...
--------------------------------------------------------------------------------
-- §5. 符号差 (P2-5 候选 ③): 律算基座维度上均为零 — Hirzebruch 一致
--------------------------------------------------------------------------------

-- 符号差 σ = 相交形式二次型的 p−q, 只在 4k 维非平凡 (Hirzebruch L 亏格)。
-- 律算基座 S² (2 维, 中维 H¹ = 0) 与 T⁶ (6 维, 奇中维 H³ 配对反对称)
-- 符号差均为 0 — 与 L 亏格在 2/6 维为零一致。

-- σ(S²) = 0 (H¹(S²) = 0, 相交形式平凡)
s2-signature : + 0 - + 0 ≡ + 0
s2-signature = refl

-- T⁶ 奇中维: b₃ = C(6,3) = C(5,2) + C(5,3) = 10 + 10 = 20 (Pascal 恒等式;
-- ZeroHomologyEquivalence 的 Betti 对称), 反对称配对 → σ = 0
t6-betti-middle : + 10 + + 10 ≡ + 20
t6-betti-middle = refl

t6-signature : + 20 - + 20 ≡ + 0
t6-signature = refl

-- 符号差与欧拉示性数之别: χ(S²) = 2 而 σ(S²) = 0 — 符号差是相交形式
-- (二次型) 的不变量, 欧拉示性数是胞腔链的交替和; 两者在 4k 维由
-- Hirzebruch 定理桥接 (σ = L 亏格), 律算基座维度上符号差全部为零,
-- 拓扑内容集中在陈数 (c₁ = χ = 2) 与 K 理论 (§4, DiscreteKTheory)。
