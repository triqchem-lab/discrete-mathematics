...
open import Sovereign.HoTT.DiscreteCubical hiding (_≡_; refl)
...
NormSq z = (ComplexAmplitude.re z * ComplexAmplitude.re z) + 
           (((+ 3) / 1) * ComplexAmplitude.im z * ComplexAmplitude.im z)
...
-- 这是离散拓扑的基本性质，不同于连续统
-- 【待核对 2026-09-08】isTopologicalBarrier: 作者注释'距离平方可能值只有0和3'
-- 但签名 ∀ distSq:ℕ 未约束值域, 对 distSq=1,2 (非可能值) 也断言成立 → 需补值域前提
-- (如 distSq ≡ 0 ⊎ 3 ≤ distSq) 或证明格点距离平方值域. 保留文本待语义核对.