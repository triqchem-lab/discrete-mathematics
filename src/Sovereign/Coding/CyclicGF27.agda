...
open import Sovereign.Algebra.GF27 using ()
  renaming (_*gf27_ to _*alg_; _+gf27_ to _+alg_; cube-mul to gf27-cube-mul)
open import Sovereign.Algebra.GF27 using
  (*gf27-assoc; cube-add; cubeMap; *gf27-distribˡ; *gf27-distribʳ; negate-⊕)
...
2u≡neg : ∀ u → u ⊕ u ≡ negate u
2u≡neg T₀ = refl
2u≡neg T₁ = refl
2u≡neg T₂ = refl

br0 : ∀ B C → B ⊕ (B ⊕ (C ⊕ C)) ≡ negate (B ⊕ C)
br0 B C = begin
  B ⊕ (B ⊕ (C ⊕ C))
    ≡⟨ sym (⊕-assoc B B (C ⊕ C)) ⟩
  (B ⊕ B) ⊕ (C ⊕ C)
    ≡⟨ cong₂ _⊕_ (2u≡neg B) (2u≡neg C) ⟩
  negate B ⊕ negate C
    ≡⟨ sym (negate-⊕ B C) ⟩
  negate (B ⊕ C)
  ∎

-- ★ 乘法桥: GF27Separation 的 *27 ≡ Algebra.GF27 的 *gf27 ★
mul-bridge : ∀ x y → x *27 y ≡ x *alg y
mul-bridge (a , b , c) (d , e , f) =
  cong₃ (λ p q r → p , q , r) e0 e1 e2
  where
    e0 : (a ⊗ d) ⊕ ((b ⊗ f) ⊕ ((b ⊗ f) ⊕ ((c ⊗ e) ⊕ (c ⊗ e))))
       ≡ (a ⊗ d) ⊕ negate ((b ⊗ f) ⊕ (c ⊗ e))
    e0 = cong ((a ⊗ d) ⊕_) (br0 (b ⊗ f) (c ⊗ e))
    e1 : (a ⊗ e) ⊕ ((b ⊗ d) ⊕ ((b ⊗ f) ⊕ ((c ⊗ e) ⊕ ((c ⊗ f) ⊕ (c ⊗ f)))))
       ≡ ((a ⊗ e) ⊕ (b ⊗ d)) ⊕ ((b ⊗ f) ⊕ ((c ⊗ e) ⊕ negate (c ⊗ f)))
    e1 = begin
      (a ⊗ e) ⊕ ((b ⊗ d) ⊕ ((b ⊗ f) ⊕ ((c ⊗ e) ⊕ ((c ⊗ f) ⊕ (c ⊗ f)))))
        ≡⟨ cong ((a ⊗ e) ⊕_) (cong ((b ⊗ d) ⊕_)
             (cong ((b ⊗ f) ⊕_) (cong ((c ⊗ e) ⊕_) (2u≡neg (c ⊗ f))))) ⟩
      (a ⊗ e) ⊕ ((b ⊗ d) ⊕ ((b ⊗ f) ⊕ ((c ⊗ e) ⊕ negate (c ⊗ f))))
        ≡⟨ sym (⊕-assoc (a ⊗ e) (b ⊗ d) ((b ⊗ f) ⊕ ((c ⊗ e) ⊕ negate (c ⊗ f)))) ⟩
      ((a ⊗ e) ⊕ (b ⊗ d)) ⊕ ((b ⊗ f) ⊕ ((c ⊗ e) ⊕ negate (c ⊗ f)))
      ∎
    e2 : (a ⊗ f) ⊕ ((b ⊗ e) ⊕ ((c ⊗ d) ⊕ (c ⊗ f)))
       ≡ ((a ⊗ f) ⊕ (b ⊗ e)) ⊕ ((c ⊗ d) ⊕ (c ⊗ f))
    e2 = sym (⊕-assoc (a ⊗ f) (b ⊗ e) ((c ⊗ d) ⊕ (c ⊗ f)))

-- ★ 立方桥: cube27 x ≡ cubeMap x ★
cube-bridge : ∀ x → cube27 x ≡ cubeMap x
cube-bridge x = begin
  (x *27 x) *27 x
    ≡⟨ cong (_*27 x) (mul-bridge x x) ⟩
  (x *alg x) *27 x
    ≡⟨ mul-bridge (x *alg x) x ⟩
  (x *alg x) *alg x
    ≡⟨ *gf27-assoc x x x ⟩
  x *alg (x *alg x)
  ∎
...
-- ★ cube-mul 构造性 ★
cube-mul-bridge : ∀ x y → cube27 (x *27 y) ≡ (cube27 x) *27 (cube27 y)
cube-mul-bridge x y = begin
  cube27 (x *27 y)
    ≡⟨ cube-bridge (x *27 y) ⟩
  cubeMap (x *27 y)
    ≡⟨ cong cubeMap (mul-bridge x y) ⟩
  cubeMap (x *alg y)
    ≡⟨ gf27-cube-mul x y ⟩
  cubeMap x *alg cubeMap y
    ≡⟨ cong₂ (λ (u v : GF27) → u *alg v) (sym (cube-bridge x)) (sym (cube-bridge y)) ⟩
  cube27 x *alg cube27 y
    ≡⟨ sym (mul-bridge (cube27 x) (cube27 y)) ⟩
  cube27 x *27 cube27 y
  ∎

-- ★ cube-sum 构造性 ★
cube-sum-bridge : ∀ x y → cube27 (x +27 y) ≡ (cube27 x) +27 (cube27 y)
cube-sum-bridge x y = begin
  cube27 (x +27 y)
    ≡⟨ cube-bridge (x +27 y) ⟩
  cubeMap (x +27 y)
    ≡⟨ cube-add x y ⟩
...
-- ★ assoc-γ 构造性 ★
assoc-γ-bridge : ∀ x y → (x *27 y) *27 γ ≡ x *27 (y *27 γ)
assoc-γ-bridge x y = begin
  (x *27 y) *27 γ
    ≡⟨ mul-bridge (x *27 y) γ ⟩
  (x *27 y) *alg γ
    ≡⟨ cong (_*alg γ) (mul-bridge x y) ⟩
  (x *alg y) *alg γ
    ≡⟨ *gf27-assoc x y γ ⟩
  x *alg (y *alg γ)
    ≡⟨ sym (cong (x *alg_) (mul-bridge y γ)) ⟩
  x *alg (y *27 γ)
    ≡⟨ sym (mul-bridge x (y *27 γ)) ⟩
  x *27 (y *27 γ)
  ∎

-- ★ distribˡ-γ 构造性 ★
distribˡ-γ-bridge : ∀ x y → γ *27 (x +27 y) ≡ (γ *27 x) +27 (γ *27 y)
distribˡ-γ-bridge x y = begin
  γ *27 (x +27 y)
    ≡⟨ mul-bridge γ (x +27 y) ⟩
  γ *alg (x +27 y)
    ≡⟨ *gf27-distribˡ γ x y ⟩
  (γ *alg x) +alg (γ *alg y)
    ≡⟨ cong₂ (λ (u v : GF27) → u +alg v) (sym (mul-bridge γ x)) (sym (mul-bridge γ y)) ⟩
  (γ *27 x) +alg (γ *27 y)
  ∎

-- ★ distribʳ-γ 构造性 ★
distribʳ-γ-bridge : ∀ x y → (x +27 y) *27 γ ≡ (x *27 γ) +27 (y *27 γ)
distribʳ-γ-bridge x y = begin
  (x +27 y) *27 γ
    ≡⟨ mul-bridge (x +27 y) γ ⟩
  (x +27 y) *alg γ
    ≡⟨ *gf27-distribʳ x y γ ⟩
  (x *alg γ) +alg (y *alg γ)
    ≡⟨ cong₂ (λ (u v : GF27) → u +alg v) (sym (mul-bridge x γ)) (sym (mul-bridge y γ)) ⟩
  (x *27 γ) +alg (y *27 γ)
  ∎
...
-- 交换律 (729 case, 生成)
-- 结构证明辅助 (2026-09-13): 交换后项序发生置换 ⇒ 需"2 项交换"与"4 项块交换"
⊕-swap2 : ∀ x y z → x ⊕ (y ⊕ z) ≡ y ⊕ (x ⊕ z)
⊕-swap2 x y z =
  trans (sym (⊕-assoc x y z))
    (trans (cong (λ u → u ⊕ z) (⊕-comm x y)) (⊕-assoc y x z))

block-swap : ∀ bf ce → bf ⊕ (bf ⊕ (ce ⊕ ce)) ≡ ce ⊕ (ce ⊕ (bf ⊕ bf))
block-swap bf ce =
  trans (sym (⊕-assoc bf bf (ce ⊕ ce)))
    (trans (⊕-comm (bf ⊕ bf) (ce ⊕ ce))
      (⊕-assoc ce ce (bf ⊕ bf)))


-- 交换律: 结构证明 (2026-09-13) —— 原 729 = 27² 条 refl 穷举子句已删除
-- 三分量各自: 交换后项序发生置换 (非逐项对齐), 用 ⊗-comm 换叶子拼写 + ⊕-swap2 / block-swap 归位
*27-comm : ∀ x y → x *₉ y ≡ y *₉ x
*27-comm (a , b , c) (d , e , f) =
  cong₃ (λ p q r → p , q , r) eq1 eq2 eq3
  where
    -- 分量 1: {ad, bf, bf, ce, ce} → {da, ec, ec, fb, fb}
    leaf1 : (b ⊗ f) ⊕ ((b ⊗ f) ⊕ ((c ⊗ e) ⊕ (c ⊗ e)))
          ≡ (f ⊗ b) ⊕ ((f ⊗ b) ⊕ ((e ⊗ c) ⊕ (e ⊗ c)))
    leaf1 = cong₂ (λ u v → u ⊕ v) (⊗-comm b f)
                  (cong₂ (λ u v → u ⊕ v) (⊗-comm b f)
                         (cong₂ (λ u v → u ⊕ v) (⊗-comm c e) (⊗-comm c e)))
    eq1 = cong₂ (λ u v → u ⊕ v) (⊗-comm a d)
                (trans leaf1 (block-swap (f ⊗ b) (e ⊗ c)))

    -- 分量 2: {ae, bd, bf, ce, cf, cf} → {bd, ae, ce, bf, cf, cf}
    rhs2 : (d ⊗ b) ⊕ ((e ⊗ a) ⊕ ((e ⊗ c) ⊕ ((f ⊗ b) ⊕ ((f ⊗ c) ⊕ (f ⊗ c)))))
         ≡ (b ⊗ d) ⊕ ((a ⊗ e) ⊕ ((c ⊗ e) ⊕ ((b ⊗ f) ⊕ ((c ⊗ f) ⊕ (c ⊗ f)))))
    rhs2 = cong₂ (λ u v → u ⊕ v) (⊗-comm d b)
             (cong₂ (λ u v → u ⊕ v) (⊗-comm e a)
               (cong₂ (λ u v → u ⊕ v) (⊗-comm e c)
                 (cong₂ (λ u v → u ⊕ v) (⊗-comm f b)
                   (cong₂ (λ u v → u ⊕ v) (⊗-comm f c) (⊗-comm f c)))))
    lhs2 : (a ⊗ e) ⊕ ((b ⊗ d) ⊕ ((b ⊗ f) ⊕ ((c ⊗ e) ⊕ ((c ⊗ f) ⊕ (c ⊗ f)))))
         ≡ (b ⊗ d) ⊕ ((a ⊗ e) ⊕ ((c ⊗ e) ⊕ ((b ⊗ f) ⊕ ((c ⊗ f) ⊕ (c ⊗ f)))))
    lhs2 =
      trans (⊕-swap2 (a ⊗ e) (b ⊗ d) ((b ⊗ f) ⊕ ((c ⊗ e) ⊕ ((c ⊗ f) ⊕ (c ⊗ f)))))
        (cong ((b ⊗ d) ⊕_)
          (cong ((a ⊗ e) ⊕_)
            (⊕-swap2 (b ⊗ f) (c ⊗ e) ((c ⊗ f) ⊕ (c ⊗ f)))))
    eq2 = trans lhs2 (sym rhs2)

    -- 分量 3: {af, be, cd, cf} → {cd, be, af, cf}
    rhs3 : (d ⊗ c) ⊕ ((e ⊗ b) ⊕ ((f ⊗ a) ⊕ (f ⊗ c)))
         ≡ (c ⊗ d) ⊕ ((b ⊗ e) ⊕ ((a ⊗ f) ⊕ (c ⊗ f)))
    rhs3 = cong₂ (λ u v → u ⊕ v) (⊗-comm d c)
             (cong₂ (λ u v → u ⊕ v) (⊗-comm e b)
               (cong₂ (λ u v → u ⊕ v) (⊗-comm f a) (⊗-comm f c)))
    lhs3 : (a ⊗ f) ⊕ ((b ⊗ e) ⊕ ((c ⊗ d) ⊕ (c ⊗ f)))
         ≡ (c ⊗ d) ⊕ ((b ⊗ e) ⊕ ((a ⊗ f) ⊕ (c ⊗ f)))
    lhs3 =
      trans (⊕-swap2 (a ⊗ f) (b ⊗ e) ((c ⊗ d) ⊕ (c ⊗ f)))
        (trans (cong ((b ⊗ e) ⊕_) (⊕-swap2 (a ⊗ f) (c ⊗ d) (c ⊗ f)))
               (⊕-swap2 (b ⊗ e) (c ⊗ d) ((a ⊗ f) ⊕ (c ⊗ f))))
    eq3 = trans lhs3 (sym rhs3)
...
cube-mul x y = cube-mul-bridge x y
cube-sum : ∀ x y → cube27 (x +₉ y) ≡ (cube27 x) +₉ (cube27 y)
cube-sum x y = cube-sum-bridge x y
assoc-γ : ∀ x y → (x *₉ y) *₉ γ ≡ x *₉ (y *₉ γ)
assoc-γ x y = assoc-γ-bridge x y
distribˡ-γ : ∀ x y → γ *₉ (x +₉ y) ≡ (γ *₉ x) +₉ (γ *₉ y)
distribˡ-γ x y = distribˡ-γ-bridge x y
distribʳ-γ : ∀ x y → (x +₉ y) *₉ γ ≡ (x *₉ γ) +₉ (y *₉ γ)
distribʳ-γ x y = distribʳ-γ-bridge x y
