...
-- 直积分解 (2026-08 修正: 原注误标为 CRT — 729 = 27×27 因子不互素,
-- 此处是 GF(3)³ × GF(3)³ 的直积/笛卡尔分解, 非中国剩余定理):
--   T⁶ ≅ GF(3)³ × GF(3)³  (729 = 27 × 27)
--   若 F 可分 F(u,v) = (F₂ u, F₃ v), 则全局矩阵是 Kronecker 积:
--   M_F[729×729] ≅ M_F₂[27×27] ⊗ M_F₃[27×27]
--   行列式乘性: det(M_F) = det(M_F₂)^27 · det(M_F₃)^27
--   (Kronecker 公式 det(A⊗B) = det(A)^m·det(B)^n, A: n×n, B: m×m;
--    原注的「det(M_F₂)³ · det(M_F₃)²」指数有误)
--   非可分 F 不满足该分解 — 全局判定仍须走 §7 的 NoZeroRow ∧ ColDistinct

-- 四极等价判定:
--   4320D 归约 + CRT 投影 + A₄ 轨道 + GF9 共轭
--   四极等价 → 可分 F 时判定复杂度从 O(729²) 降至 O(27² + 27²)
--   (不可分 F 仍须 729×729 全局判定; 结构性判据 NoZeroRow ∧ ColDistinct 见 §7)
