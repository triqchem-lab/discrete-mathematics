...
open import Sovereign.Base.Trit using (Trit; T₀; T₁; T₂; _⊕_; _⊗_; negate; tritToFin3; ⊗-distribˡ-⊕)
...
--------------------------------------------------------------------------------
-- §4. 特征是群同态: χ(p·q) = χ(p)·χ(q)
-- (构造性证明见文件末尾 §4′, 替代原 1728 case 穷举)
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- §4′. 特征同态 (构造性: 双通道分解, 替代原 1728 case 穷举)
--
-- χ(u,v)(p·q) = χ(u,v)(p)·χ(u,v)(q)
-- 分解: 加法通道 (tritToZ 保 ⊕) + 相位通道 (alphaToZ 保 mulAlpha, alpha-pow 保 mulAlpha)
--------------------------------------------------------------------------------

open ≡-Reasoning

tritToZ-add : ∀ u t → tritToZ (u ⊕ t) ≡ tritToZ u *ᶻ tritToZ t
tritToZ-add T₀ t = sym (z1-lunit (tritToZ t))
tritToZ-add T₁ T₀ = refl
tritToZ-add T₁ T₁ = refl
tritToZ-add T₁ T₂ = refl
tritToZ-add T₂ T₀ = refl
tritToZ-add T₂ T₁ = refl
...
alphaToZ-mul : ∀ v w → alphaToZ (mulAlpha v w) ≡ alphaToZ v *ᶻ alphaToZ w
alphaToZ-mul a0 w = sym (z1-lunit (alphaToZ w))
alphaToZ-mul a1 a0 = refl
alphaToZ-mul a1 a1 = refl
alphaToZ-mul a1 a2 = refl
alphaToZ-mul a1 a3 = refl
alphaToZ-mul a2 a0 = refl
alphaToZ-mul a2 a1 = refl
alphaToZ-mul a2 a2 = refl
alphaToZ-mul a2 a3 = refl
alphaToZ-mul a3 a0 = refl
alphaToZ-mul a3 a1 = refl
alphaToZ-mul a3 a2 = refl
alphaToZ-mul a3 a3 = refl




tritToZ-dist : ∀ u t t' → tritToZ (u ⊗ (t ⊕ t')) ≡ tritToZ (u ⊗ t) *ᶻ tritToZ (u ⊗ t')
tritToZ-dist u t t' = trans (cong tritToZ (⊗-distribˡ-⊕ u t t')) (tritToZ-add (u ⊗ t) (u ⊗ t'))


-- ★ 相位层恒等式 (纯 AlphaPower, 无 ℚ): (a·b)^v = a^v · b^v ★
-- C₄ 交换群, 用 mulAlpha-assoc/comm 推导
alpha-pow-mul : ∀ v a b →
  alpha-pow v (mulAlpha a b) ≡ mulAlpha (alpha-pow v a) (alpha-pow v b)
alpha-pow-mul a0 a b = refl
alpha-pow-mul a1 a b = refl
alpha-pow-mul a2 a b = begin
  mulAlpha (mulAlpha a b) (mulAlpha a b)
    ≡⟨ mulAlpha-assoc a b (mulAlpha a b) ⟩
  mulAlpha a (mulAlpha b (mulAlpha a b))
    ≡⟨ cong (λ x → mulAlpha a x) (sym (mulAlpha-assoc b a b)) ⟩
  mulAlpha a (mulAlpha (mulAlpha b a) b)
    ≡⟨ cong (λ x → mulAlpha a (mulAlpha x b)) (mulAlpha-comm b a) ⟩
  mulAlpha a (mulAlpha (mulAlpha a b) b)
    ≡⟨ cong (λ x → mulAlpha a x) (mulAlpha-assoc a b b) ⟩
  mulAlpha a (mulAlpha a (mulAlpha b b))
    ≡⟨ sym (mulAlpha-assoc a a (mulAlpha b b)) ⟩
  mulAlpha (mulAlpha a a) (mulAlpha b b)
  ∎
alpha-pow-mul a3 a b = begin
  mulAlpha (mulAlpha a b) (mulAlpha (mulAlpha a b) (mulAlpha a b))
    ≡⟨ cong (λ x → mulAlpha (mulAlpha a b) x) (alpha-pow-mul a2 a b) ⟩
  mulAlpha (mulAlpha a b) (mulAlpha (mulAlpha a a) (mulAlpha b b))
    ≡⟨ mulAlpha-assoc a b (mulAlpha (mulAlpha a a) (mulAlpha b b)) ⟩
  mulAlpha a (mulAlpha b (mulAlpha (mulAlpha a a) (mulAlpha b b)))
    ≡⟨ cong (λ x → mulAlpha a x) (sym (mulAlpha-assoc b (mulAlpha a a) (mulAlpha b b))) ⟩
  mulAlpha a (mulAlpha (mulAlpha b (mulAlpha a a)) (mulAlpha b b))
    ≡⟨ cong (λ x → mulAlpha a (mulAlpha x (mulAlpha b b))) (mulAlpha-comm b (mulAlpha a a)) ⟩
  mulAlpha a (mulAlpha (mulAlpha (mulAlpha a a) b) (mulAlpha b b))
    ≡⟨ cong (λ x → mulAlpha a x) (mulAlpha-assoc (mulAlpha a a) b (mulAlpha b b)) ⟩
  mulAlpha a (mulAlpha (mulAlpha a a) (mulAlpha b (mulAlpha b b)))
    ≡⟨ sym (mulAlpha-assoc a (mulAlpha a a) (mulAlpha b (mulAlpha b b))) ⟩
  mulAlpha (mulAlpha a (mulAlpha a a)) (mulAlpha b (mulAlpha b b))
  ∎
...
dc-character-hom (u , v) (t , a) (t' , b) = begin
  dc-character (u , v) (mixedOp (t , a) (t' , b))
    ≡⟨⟩
  tritToZ (u ⊗ (t ⊕ t')) *ᶻ alphaToZ (alpha-pow v (mulAlpha a b))
    ≡⟨ cong₂ _*ᶻ_ (tritToZ-dist u t t') (cong alphaToZ (alpha-pow-mul v a b)) ⟩
  (tritToZ (u ⊗ t) *ᶻ tritToZ (u ⊗ t')) *ᶻ alphaToZ (mulAlpha (alpha-pow v a) (alpha-pow v b))
    ≡⟨ cong ((tritToZ (u ⊗ t) *ᶻ tritToZ (u ⊗ t')) *ᶻ_) (alphaToZ-mul (alpha-pow v a) (alpha-pow v b)) ⟩
  (tritToZ (u ⊗ t) *ᶻ tritToZ (u ⊗ t')) *ᶻ (alphaToZ (alpha-pow v a) *ᶻ alphaToZ (alpha-pow v b))
    ≡⟨ *ᶻ-assoc (tritToZ (u ⊗ t)) (tritToZ (u ⊗ t')) (alphaToZ (alpha-pow v a) *ᶻ alphaToZ (alpha-pow v b)) ⟩
  tritToZ (u ⊗ t) *ᶻ (tritToZ (u ⊗ t') *ᶻ (alphaToZ (alpha-pow v a) *ᶻ alphaToZ (alpha-pow v b)))
    ≡⟨ cong (tritToZ (u ⊗ t) *ᶻ_) (sym (*ᶻ-assoc (tritToZ (u ⊗ t')) (alphaToZ (alpha-pow v a)) (alphaToZ (alpha-pow v b)))) ⟩
  tritToZ (u ⊗ t) *ᶻ ((tritToZ (u ⊗ t') *ᶻ alphaToZ (alpha-pow v a)) *ᶻ alphaToZ (alpha-pow v b))
    ≡⟨ cong (λ x → tritToZ (u ⊗ t) *ᶻ (x *ᶻ alphaToZ (alpha-pow v b))) (*ᶻ-comm (tritToZ (u ⊗ t')) (alphaToZ (alpha-pow v a))) ⟩
  tritToZ (u ⊗ t) *ᶻ ((alphaToZ (alpha-pow v a) *ᶻ tritToZ (u ⊗ t')) *ᶻ alphaToZ (alpha-pow v b))
    ≡⟨ cong (tritToZ (u ⊗ t) *ᶻ_) (*ᶻ-assoc (alphaToZ (alpha-pow v a)) (tritToZ (u ⊗ t')) (alphaToZ (alpha-pow v b))) ⟩
  tritToZ (u ⊗ t) *ᶻ (alphaToZ (alpha-pow v a) *ᶻ (tritToZ (u ⊗ t') *ᶻ alphaToZ (alpha-pow v b)))
    ≡⟨ sym (*ᶻ-assoc (tritToZ (u ⊗ t)) (alphaToZ (alpha-pow v a)) (tritToZ (u ⊗ t') *ᶻ alphaToZ (alpha-pow v b))) ⟩
  (tritToZ (u ⊗ t) *ᶻ alphaToZ (alpha-pow v a)) *ᶻ (tritToZ (u ⊗ t') *ᶻ alphaToZ (alpha-pow v b))
    ≡⟨⟩
  dc-character (u , v) (t , a) *ᶻ dc-character (u , v) (t' , b)
  ∎

--------------------------------------------------------------------------------
-- §5′. 反射 ρ 特征 (构造性: 双通道 negate/inv, 替代原 144 case 穷举)
--------------------------------------------------------------------------------

-- 通道 1: tritToZ 对 negate (9 case)
tritToZ-neg : ∀ u t → tritToZ (u ⊗ negate t) ≡ conjᶻ (tritToZ (u ⊗ t))
tritToZ-neg T₀ t = refl
tritToZ-neg T₁ T₀ = refl
tritToZ-neg T₁ T₁ = refl
tritToZ-neg T₁ T₂ = refl
tritToZ-neg T₂ T₀ = refl
...
-- 通道 2: alphaToZ 对 alphaInv (4 case)
alphaToZ-inv : ∀ a → alphaToZ (alphaInv a) ≡ conjᶻ (alphaToZ a)
alphaToZ-inv a0 = refl
alphaToZ-inv a1 = refl
alphaToZ-inv a2 = refl
alphaToZ-inv a3 = refl

-- alphaInv 保 mulAlpha (16 case)
mulAlpha-inv : ∀ x y → mulAlpha (alphaInv x) (alphaInv y) ≡ alphaInv (mulAlpha x y)
mulAlpha-inv a0 y = refl
mulAlpha-inv a1 a0 = refl
mulAlpha-inv a1 a1 = refl
mulAlpha-inv a1 a2 = refl
mulAlpha-inv a1 a3 = refl
mulAlpha-inv a2 a0 = refl
mulAlpha-inv a2 a1 = refl
mulAlpha-inv a2 a2 = refl
mulAlpha-inv a2 a3 = refl
mulAlpha-inv a3 a0 = refl
mulAlpha-inv a3 a1 = refl
mulAlpha-inv a3 a2 = refl
mulAlpha-inv a3 a3 = refl

-- 通道 3: alpha-pow 与 alphaInv 交换
alpha-pow-inv : ∀ v a → alpha-pow v (alphaInv a) ≡ alphaInv (alpha-pow v a)
alpha-pow-inv a0 a = refl
alpha-pow-inv a1 a = refl
alpha-pow-inv a2 a = mulAlpha-inv a a
alpha-pow-inv a3 a =
  trans (cong (λ x → mulAlpha (alphaInv a) x) (mulAlpha-inv a a))
        (mulAlpha-inv a (mulAlpha a a))

-- ★ rho-character 构造性证明 ★
rho-character : ∀ (idx : CharacterIndex) (p : DuodecPoint) →
  dc-character idx (rho p) ≡ conjᶻ (dc-character idx p)
rho-character (u , v) (t , a) = begin
  dc-character (u , v) (rho (t , a))
    ≡⟨⟩
  tritToZ (u ⊗ negate t) *ᶻ alphaToZ (alpha-pow v (alphaInv a))
    ≡⟨ cong₂ _*ᶻ_ (tritToZ-neg u t) (cong alphaToZ (alpha-pow-inv v a)) ⟩
  conjᶻ (tritToZ (u ⊗ t)) *ᶻ alphaToZ (alphaInv (alpha-pow v a))
    ≡⟨ cong (conjᶻ (tritToZ (u ⊗ t)) *ᶻ_) (alphaToZ-inv (alpha-pow v a)) ⟩
  conjᶻ (tritToZ (u ⊗ t)) *ᶻ conjᶻ (alphaToZ (alpha-pow v a))
    ≡⟨ sym (conj-*ᶻ (tritToZ (u ⊗ t)) (alphaToZ (alpha-pow v a))) ⟩
  conjᶻ (tritToZ (u ⊗ t) *ᶻ alphaToZ (alpha-pow v a))
    ≡⟨⟩
  conjᶻ (dc-character (u , v) (t , a))
  ∎
